...
import org.apache.lucene.queryparser.classic.CustomBooleanQuery;
...
public class CustomQueryStringBuilder extends AbstractQueryBuilder<CustomQueryStringBuilder> {


    /**
     * 定制custom,进入分词器之前初次分词
     */
    public static final String NAME = "custom_query_string";
    ...
    public CustomQueryStringBuilder(String queryString) {
        if (queryString == null) {
     ...
     * Read from a stream.
     */
    public CustomQueryStringBuilder(StreamInput in) throws IOException {
        super(in);
     ...
     */
    public CustomQueryStringBuilder defaultField(String defaultField) {
     ...
     */
    public CustomQueryStringBuilder field(String field) {
     ...
     */
    public CustomQueryStringBuilder field(String field, float boost) {
     ...
     */
    public CustomQueryStringBuilder fields(Map<String, Float> fields) {
     ...
     */
    public CustomQueryStringBuilder type(MultiMatchQueryBuilder.Type type) {
        this.type = type;
        return this;
    }
     ...
     */
    public CustomQueryStringBuilder tieBreaker(float tieBreaker) {
     ...
     */
    public CustomQueryStringBuilder defaultOperator(Operator defaultOperator) {
     ...
     */
    public CustomQueryStringBuilder analyzer(String analyzer) {
     ...
     * defined for it, then it will be used automatically. Defaults to the smart search analyzer.
     */
    public CustomQueryStringBuilder quoteAnalyzer(String quoteAnalyzer) {
     ...
     */
    public CustomQueryStringBuilder maxDeterminizedStates(int maxDeterminizedStates) {
     ...
     */
    public CustomQueryStringBuilder allowLeadingWildcard(Boolean allowLeadingWildcard) {
     ...
     */
    public CustomQueryStringBuilder enablePositionIncrements(boolean enablePositionIncrements) {
     ...
     */
    public CustomQueryStringBuilder fuzziness(Fuzziness fuzziness) {
     ...
     */
    public CustomQueryStringBuilder fuzzyPrefixLength(int fuzzyPrefixLength) {
    ...
    public CustomQueryStringBuilder fuzzyMaxExpansions(int fuzzyMaxExpansions) {
    ...
    public CustomQueryStringBuilder fuzzyRewrite(String fuzzyRewrite) {
     ...
     */
    public CustomQueryStringBuilder phraseSlop(int phraseSlop) {
    ...
    public CustomQueryStringBuilder rewrite(String rewrite) {
     ...
     */
    public CustomQueryStringBuilder analyzeWildcard(Boolean analyzeWildcard) {
    ...
    public CustomQueryStringBuilder minimumShouldMatch(String minimumShouldMatch) {
     ...
     */
    public CustomQueryStringBuilder quoteFieldSuffix(String quoteFieldSuffix) {
     ...
     */
    public CustomQueryStringBuilder lenient(Boolean lenient) {
     ...
     */
    public CustomQueryStringBuilder timeZone(String timeZone) {
        if (timeZone != null) {
    ...
    public CustomQueryStringBuilder timeZone(ZoneId timeZone) {
     ...
     */
    public CustomQueryStringBuilder escape(boolean escape) {
        this.escape = escape;
        return this;
    }
    ...
    public CustomQueryStringBuilder autoGenerateSynonymsPhraseQuery(boolean value) {
     ...
     */
    public CustomQueryStringBuilder fuzzyTranspositions(boolean fuzzyTranspositions) {
    ...
    public static CustomQueryStringBuilder fromXContent(XContentParser parser) throws IOException {
        ...
        int maxDeterminizedStates = CustomQueryStringBuilder.DEFAULT_MAX_DETERMINED_STATES;
        boolean enablePositionIncrements = CustomQueryStringBuilder.DEFAULT_ENABLE_POSITION_INCREMENTS;
        boolean escape = CustomQueryStringBuilder.DEFAULT_ESCAPE;
        int fuzzyPrefixLength = CustomQueryStringBuilder.DEFAULT_FUZZY_PREFIX_LENGTH;
        int fuzzyMaxExpansions = CustomQueryStringBuilder.DEFAULT_FUZZY_MAX_EXPANSIONS;
        int phraseSlop = CustomQueryStringBuilder.DEFAULT_PHRASE_SLOP;
        ...
        Operator defaultOperator = CustomQueryStringBuilder.DEFAULT_OPERATOR;
        String timeZone = null;
        Fuzziness fuzziness = CustomQueryStringBuilder.DEFAULT_FUZZINESS;
        ...
        while ((token = parser.nextToken()) != XContentParser.Token.END_OBJECT) {
            if (token == XContentParser.Token.FIELD_NAME) {
                ...
                if (FIELDS_FIELD.match(currentFieldName, parser.getDeprecationHandler())) {
                    ...
                    throw new ParsingException(parser.getTokenLocation(), "[" + CustomQueryStringBuilder.NAME +
        ...
        if (queryString == null) {
            throw new ParsingException(parser.getTokenLocation(), "[" + CustomQueryStringBuilder.NAME + "] must be provided with a [query]");
        }

        CustomQueryStringBuilder queryStringQuery = new CustomQueryStringBuilder(queryString);
    ...
    @Override
    protected boolean doEquals(CustomQueryStringBuilder other) {
    ...
    protected Query doToQuery(QueryShardContext context) throws IOException {
        ...
        if (fieldsAndWeights.size() > 0 && defaultField() != null) {
        ...
        Query query;
        try {
            //TODO  rewrittenQueryString 重写
            rewrittenQueryString = convertQueryString(rewrittenQueryString, customQueryParser, convertDefaultField);
            //默认 定义为standard 做单字分词
            queryParser.setAnalyzer(new StandardAnalyzer());
    ...
    String convertQueryString(String rewrittenQueryString, CustomQueryParser customQueryParser, String convertDefaultField) throws ParseException {
        Query convertQuery;
        convertQuery = customQueryParser.parse(rewrittenQueryString);
        if (convertQuery instanceof BooleanQuery) {
            BooleanQuery booleanQuery = (BooleanQuery) convertQuery;
            CustomBooleanQuery customBooleanQuery = new CustomBooleanQuery(booleanQuery);
            rewrittenQueryString = customBooleanQuery.toString(convertDefaultField);
        } else if (convertQuery instanceof PhraseQuery) {
            StringBuilder buffer = new StringBuilder();
            PhraseQuery phraseQuery = (PhraseQuery) convertQuery;
            buffer.append("(");
            ...
            buffer.append(")");
            rewrittenQueryString = buffer.toString();
            System.out.println(rewrittenQueryString);
        } else {
            rewrittenQueryString = convertQuery.toString();
            if (convertDefaultField == null) {
                convertDefaultField = "";
            }
            if (rewrittenQueryString.contains(convertDefaultField)) {
                rewrittenQueryString = rewrittenQueryString.substring(rewrittenQueryString.indexOf(":") + 1);
            }
        }
        return rewrittenQueryString;
    }

    CustomQueryParser parseQuery(String defaultField, Analyzer analyzer) {
        return new CustomQueryParser(defaultField, analyzer);
    }
